...
Outlook / Thunderbird / Apple Mail ◀──IMAPS:993 (TLS)──────────── Dovecot
                                   ──SMTP:587 (STARTTLS, mailbox login)──▶ BunMail submission
...
- **Sending**: the SMTP submission server (#120) accepts `user@domain` +
  mailbox password in addition to API keys. The `From` is pinned to the
...
| `MAILBOX_SMTP_HOST`          | `MAIL_HOSTNAME`| Shown to users (port = `SMTP_SUBMISSION_PORT`). |
...
points `SMTP_SUBMISSION_TLS_CERT/KEY` at the acme volume so STARTTLS is on.
...
| Outgoing (SMTP)  | `smtp.example.com`, port 587, STARTTLS  |
...
- Submission (587) requires AUTH; mailbox logins may only send `From` their
  own address; per-IP failed-AUTH lockout (#133) applies to mailbox logins.
...
swaks --to you@gmail.com --from mike@example.com --server smtp.example.com:587 \
      --tls --auth-user mike@example.com --auth-password 'password'
```
